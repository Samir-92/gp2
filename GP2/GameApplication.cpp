...
bool CGameApplication::initGame()
{
	...
	pMaterial->setAmbientMaterialColour(D3DXCOLOR(1.0f,1.5f,0.2f,1.0f));
	...
	//Create GameObject
	CGameObject *pTestGameObject2=new CGameObject();
	//Set the name
	pTestGameObject2->setName("Test");
	//Position
	pTestGameObject2->getTransform()->setPosition(0.0f,1.0f,10.0f);
	//create material
	...
	pMaterial2->setAmbientMaterialColour(D3DXCOLOR(1.0f,1.5f,0.2f,1.0f));
	pTestGameObject2->addComponent(pMaterial2);
	pMaterial2->loadDiffuseTexture("armoredrecon_diff.png");

	//Create Mesh
	CMeshComponent *pMesh2=modelloader.loadModelFromFile(m_pD3D10Device,"armoredrecon.fbx");
	//CMeshComponent *pMesh=modelloader.createCube(m_pD3D10Device,10.0f,10.0f,10.0f);
	pMesh2->SetRenderingDevice(m_pD3D10Device);
	pTestGameObject2->addComponent(pMesh2);
	//add the game object
	m_pGameObjectManager->addGameObject(pTestGameObject2);
